...
Film* creationFilm(std::string titre, std::string realisateur, int anneeSortie, int recette, int nActeurs) {
    return new Film(titre, realisateur, anneeSortie, recette, nActeurs);
}

Film* lireFilm(istream& fichier, ListeFilms* liste) {
    std::string titre = lireString(fichier);
    std::string realisateur = lireString(fichier);
    int anneeSortie = lireUint16(fichier);
    int recette = lireUint16(fichier);
    int nActeurs = lireUint8(fichier);
    Film* film = creationFilm(titre, realisateur, anneeSortie, recette, nActeurs);
...
int main() {
    ...
    ListeFilms liste = creerListe("Y:\\inf1015\\TP_INF_1015\\TP2\\TD2-H25-Fichiers\\films.bin");
